...
func Handle(admin bool) gin.HandlerFunc {
	return func(c *gin.Context) {
			...
			Named("bottomBarElse", Div(NewOpt().H("parent.h-next.h"),
				...
				Named("leftRightBarElse", Div(NewOpt().X("prev.x2").W("parent.w-prev.w-next.w"),
					Named("bottomViewElse", Div(NewOpt().H("next.y+next.h/2").V("!!root.leftView || !!root.rightView"),
						Named("leftView", Div(NewOpt().W("next.x").V("!!root.leftView").BorderColor(ColorGray235).BorderRight(1),
							ProjectView(),
						)),
						Named("rightViewBar", VBar(NewBarOpt().BgColor(ColorGray247).X("prev.v ? (next.v ? parent.w/2 : parent.w-.w/2) : -.w/2").V("next.v && prev.v"))),
						Named("rightView", Div(NewOpt().X("prev.x2").W("parent.w-.x").V("!!root.rightView").BgColor(ColorGray247).BorderLeft(1).BorderColor(ColorGray235),
							NoteView(),
							ChatView(),
						)),
					)),
		...
		root.SetProperty("admin", "false")
		if admin {
			root.SetProperty("admin", "true")
		}
