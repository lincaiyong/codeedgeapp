...
func NoteView() *Element {
	return Div(NewOpt().V("root.rightView === 'note'"),
		Div(NewOpt().H("33").BgColor(ColorGray247).BorderBottom(1).BorderColor(ColorGray235),
			...
			Button(NewButtonOpt().X("next.x-.w").Y("next.y").Svg(SvgSave).OnClick("note_onSave")),
		...
		Div(NewOpt().Y("prev.y2").H("parent.h-.y"),
			Named("note", Note(NewNoteOpt().OnOpenLink("(filePath, lineNumber, patch) => project_openFile({filePath, lineNumber, patch})"))),
		),
	)
}
