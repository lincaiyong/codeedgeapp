...
func LeftBar(opt *Opt) *Element {
	return Div(opt,
		Named("projectBtn", ToolButton(NewButtonOpt().Svg(SvgProject).X("parent.w/2-.w/2-0.5").Y(".x").Selected("root.leftView === 'project'").OnClick("sidebar_onClickProject"))),
		//ToolButton(NewButtonOpt().Svg(SvgSearch).X("prev.x").Y("prev.y2 + 8").Selected("root.showSearch").OnClick("sidebar_onClickSearch")),
		...
		ToolButton(NewButtonOpt().Svg(SvgSearch).X("next.x").Y("next.y-8-.h").Selected("root.bottomView === 'search'").OnClick("sidebar_onClickSearch")),
		ToolButton(NewButtonOpt().Svg(SvgImportDataCell).X("parent.w/2-.w/2-0.5").Y("parent.h-.h-.x").Selected("root.bottomView === 'data'").OnClick("sidebar_onClickData")),
	)
}

func RightBar(opt *Opt) *Element {
	return Div(opt,
		ToolButton(NewButtonOpt().Svg(SvgChangedFile).X("parent.w/2-.w/2-0.5").Y(".x").Flag("false").Selected("root.rightView == 'note'").OnClick("sidebar_onClickNote")),
		ToolButton(NewButtonOpt().Svg(SvgAIChat).V("root.admin").X("prev.x").Y("prev.y2 + 8").Selected("root.rightView == 'chat'").OnClick("sidebar_onClickChat")),
		//ToolButton(NewButtonOpt().Svg(SvgDatabase).X("prev.x").Y("prev.y2 + 8")),
		//ToolButton(NewButtonOpt().Svg(SvgBookMarks).X("prev.x").Y("prev.y2 + 8").Selected("root.rightView === 'bookmark'").OnClick("sidebar_clickBookmark")),
	)
}
